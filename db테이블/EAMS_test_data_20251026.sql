...
-- EAMS Test Data Inserts (2025-10-26, Asia/Seoul)
-- Assumes all tables from the provided DDL already exist.
...
INSERT INTO `user` (user_id, user_name, birth_date, gender, phone, address, account_number, department_id) VALUES
  (1, '김교수', '1975-05-12', 'M', '010-1234-5678', '서울시 강남구', '1234-5678-9012-3456', 101),
  (2, '이학생', '2003-03-21', 'F', '010-2345-6789', '서울시 광진구', '2345-6789-0123-4567', 101),
  (3, '박학생', '2002-11-02', 'M', '010-3456-7890', '서울시 송파구', NULL,                   102),
  (4, '최직원', '1988-08-08', 'F', '010-4567-8901', '서울시 동대문구', '3456-7890-1234-5678', 103),
  (5, '정조교', '1997-09-17', 'F', '010-5678-9012', '서울시 성동구',  '4567-8901-2345-6789', 101),
  (6, '오사용자', '1990-01-10', 'M', '010-6789-0123', '경기도 성남시', NULL,               103);
...
-- 4) Lectures
INSERT INTO Lecture
  (lecture_id, lecture_name, enrollment_count, credit, classroom, professor_id, lecture_year, lecture_semester, lecture_hours)
VALUES
  (1, '자료구조',  2, 3, 'D201', 1, '2025년', 2, 3),
  (2, '전자회로',  1, 3, 'E103', 1, '2025년', 2, 3);

-- 5) enrollment
INSERT INTO enrollment (enrollment_id, lecture_id, student_id) VALUES
  (1, 1, 2),
  (2, 2, 3);
...
-- 13) Chatroom & members & messages  (UPDATED: member_id → FK user.user_id)
-- 방 생성
INSERT INTO Chatroom (chatroom_id, chatroom_name, is_group, last_message_time) VALUES
  (1, '자료구조 스터디', 't', '2025-10-10');

-- 채팅방 멤버 (복합 PK: chatroom_id + member_id, FK: member_id → user.user_id)
-- 사용자: (1=김교수), (2=이학생), (5=정조교)
INSERT INTO Chatroom_List (chatroom_id, member_id) VALUES
  (1, 1),
  (1, 2),
  (1, 5);

-- 메시지 (author_id → user.user_id)
INSERT INTO chatting (message_id, chatroom_id, created_date, content, file_path, author_id) VALUES
  (1, 1, '2025-10-10', '오늘 7시에 만나요', NULL, 2),
  (2, 1, '2025-10-10', '네, 연구실 앞에서 봅시다.', NULL, 1),
  (3, 1, '2025-10-10', '제가 과제 설명도 도와드릴게요.', NULL, 5);


-- 14) Leave_of_Absence (employee & student composite PK)
