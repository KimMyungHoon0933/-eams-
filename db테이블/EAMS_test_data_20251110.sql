...
-- 4) Lectures
INSERT INTO Lecture
  (lecture_id, lecture_name, lecture_year, enrollment_count, credit,
   lecture_semester, day_of_week, start_hours, end_hours, room, professor_id)
VALUES
  (1, '자료구조', '2025', 2, 3, 2, '월', '18:00:00', '19:30:00', 'D201', 1),
  (2, '전자회로', '2025', 1, 3, 2, '수', '15:00:00', '16:30:00', 'E103', 1);
-- 5) enrollment
...
-- 7) grade
INSERT INTO grades (enrollment_id, grade, midterm_score, final_score) VALUES
  (1, 'A',  92.50, 95.00),
  (2, 'B+', 84.00, 88.50);
...
-- 컬럼: (leave_id, student_id, reason, applied_at, leave_year, leave_semester, leave_type, is_on_leave)
-- leave_term은 VIRTUAL 생성칼럼이라 INSERT 대상이 아닙니다.
INSERT INTO Leave_of_Absence
  (leave_id, student_id, reason, applied_at, leave_year, leave_semester, leave_type, is_on_leave)
VALUES
  (1, 3, '개인 사유로 2학기 휴학 신청', '2025-09-15 10:00:00', 2025, 2, '일반휴학', 'T');

-- 15) back_school  ✅ (신규 복학 테이블 데이터)
--    컬럼: (return_id, leave_id, manager, return_reason, return_date, is_returned)
--    manager 는 직원 FK(NULL 허용). 예시로 employee_id=4를 담당자로 지정.

INSERT INTO back_school (return_id, leave_id, manager, return_reason, return_date, is_returned) VALUES
  (1, 1, 4, '복학 승인 처리', '2026-03-01', 't');
