...
CREATE TABLE `Leave_of_Absence` (
  `leave_id`    INT UNSIGNED  NOT NULL AUTO_INCREMENT,   -- 휴학 신청 PK
  `student_id`  INT UNSIGNED  NOT NULL,                  -- 학생 FK
  `reason`      TEXT          NOT NULL,                  -- 휴학 사유
  `leave_date`  DATE          NOT NULL,                  -- 휴학 날짜(시작일/처리일 중 선택해 사용)
...
CREATE TABLE `back_school` (
  ...
  CONSTRAINT `FK_RFL_EMP`
    FOREIGN KEY (`담당자`) REFERENCES `employee`(`employee_id`)
