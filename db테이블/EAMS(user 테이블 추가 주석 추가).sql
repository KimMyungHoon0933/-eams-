...
CREATE TABLE `Lecture` (
  `lecture_id`       INT UNSIGNED NOT NULL AUTO_INCREMENT, -- PK
  `lecture_name`     VARCHAR(50)  NULL,                    -- 과목명
  `enrollment_count` INT          NOT NULL,                 -- 수강인원
  `credit`           TINYINT      NOT NULL,                 -- 학점(0~9)
  `classroom`        VARCHAR(50)  NOT NULL,                 -- 강의실
  `professor_id`     INT UNSIGNED NOT NULL,                 -- 담당교수(FK)
  `lecture_year`     CHAR(5)      NOT NULL,                 -- '0000년' 형식
  `lecture_semester` TINYINT      NOT NULL,                 -- 1~2학기
  `lecture_hours`    TINYINT      NOT NULL,                 -- 시수(1~9)
  `day_of_week`      CHAR(1)      NOT NULL COMMENT '요일(월~일)', -- ← 추가
