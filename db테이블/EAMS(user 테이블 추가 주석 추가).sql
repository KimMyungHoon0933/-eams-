...
CREATE TABLE `user` (
  ...
  `user_name`      VARCHAR(50)   NOT NULL,                        -- 이름, NULL 금지, 1~50자
  `birth_date`     DATE          NOT NULL,                        -- 생년월일, NULL 금지
  `gender`         VARCHAR(1)    NOT NULL,                        -- 성별, NULL 금지, 정확히 1글자(CHECK)
  `phone`          VARCHAR(13)   NOT NULL,                        -- 연락처, NULL 금지, 패턴 000-0000-0000 (CHECK REGEXP)
  `address`        VARCHAR(100)  NULL,                            -- 주소, NULL 허용
  `account_number` CHAR(43)      NULL,                            -- 계좌, NULL 허용, 패턴 0000-0000-0000-0000 (CHECK REGEXP)
  `department_id`  INT           NOT NULL,                        -- 소속 학과, NULL 금지, FK → department.department_id
  ...
  CONSTRAINT `FK_USER_DEPARTMENT`
    FOREIGN KEY (`department_id`) REFERENCES `department`(`department_id`)  -- ON DELETE/UPDATE 기본(RESTRICT)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
