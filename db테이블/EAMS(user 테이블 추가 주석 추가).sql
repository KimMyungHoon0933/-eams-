...
CREATE TABLE `Lecture` (
  `lecture_id`       INT UNSIGNED      NOT NULL AUTO_INCREMENT COMMENT 'PK',
  `lecture_name`     VARCHAR(100)      NOT NULL               COMMENT '강의명',
  `lecture_year`     CHAR(4)           NOT NULL               COMMENT '개설년도(YYYY)',
  `enrollment_count` INT          NOT NULL,                 -- 수강인원,
  `credit`           TINYINT      NOT NULL,                 -- 학점(0~9)
  `lecture_semester` TINYINT UNSIGNED  NOT NULL               COMMENT '학기(1=1학기, 2=2학기)',
  `day_of_week`      CHAR(1)           NOT NULL               COMMENT '요일(월~일)',
  `start_hours`      TIME              NOT NULL               COMMENT '시작시간(HH:MM:SS)',
  `end_hours`        TIME              NOT NULL               COMMENT '종료시간(HH:MM:SS)',
  `room`             VARCHAR(50)       NULL                   COMMENT '강의실(옵션)',
  `professor_id`     INT UNSIGNED      NULL                   COMMENT '담당교수(옵션, user.user_id 참조)',

  CONSTRAINT `PK_LECTURE` PRIMARY KEY (`lecture_id`),
  CONSTRAINT `CK_LECTURE_DAY`       CHECK (`day_of_week` IN ('월','화','수','목','금','토','일')),
  CONSTRAINT `CK_LECTURE_CREDIT`    CHECK (`credit` BETWEEN 0 AND 9),
  CONSTRAINT `CK_LECTURE_YEAR_FMT`  CHECK (`lecture_year` REGEXP '^[0-9]{4}$'),
  CONSTRAINT `CK_LECTURE_SEMESTER`  CHECK (`lecture_semester` BETWEEN 1 AND 2),
  -- 기본 검증
  CONSTRAINT `CK_LECTURE_TIME_RANGE`   CHECK (`start_hours` < `end_hours`),

  -- 시간표 중복 방지(같은 년/학기/요일/강의실 동일 시간대 충돌 최소화)
  KEY `IX_LECTURE_YEAR_SEM_DAY` (`lecture_year`,`lecture_semester`,`day_of_week`),
  KEY `IX_LECTURE_PROF` (`professor_id`),
  UNIQUE KEY `UQ_TIMEBLOCK_ROOM`
    (`lecture_year`,`lecture_semester`,`day_of_week`,`room`,`start_hours`,`end_hours`)
...
CREATE TABLE `enrollment` (
  ...
  CONSTRAINT `CK_ENROLL_YEAR_FMT` CHECK (REGEXP_LIKE(`lecture_year`, '^[0-9]{4}$')),
  CONSTRAINT `CK_ENROLL_SEMESTER` CHECK (`lecture_semester` BETWEEN 1 AND 2),
  CONSTRAINT `CK_ENROLL_HOURS`    CHECK (`lecture_hours` BETWEEN 1 AND 9),
...
CREATE TABLE `attendance` (
  ...
  `absent_hours`    INT          NOT NULL,                          -- 결석(지각) '시간', 0~98
  `absent_minutes`  TINYINT      NOT NULL DEFAULT 0,                -- 결석(지각) '분', 0~59

  CONSTRAINT `PK_ATTENDANCE` PRIMARY KEY (`attendance_date`, `enrollment_id`),
  CONSTRAINT `CK_ATTENDANCE_ABSENT_HOURS`
    CHECK (`absent_hours` BETWEEN 0 AND 98),
  CONSTRAINT `CK_ATTENDANCE_ABSENT_MINUTES`
    CHECK (`absent_minutes` BETWEEN 0 AND 59),
...
CREATE TABLE `grades` (
  `enrollment_id` INT UNSIGNED NOT NULL COMMENT '수강신청 PK 참조',
  `grade`         VARCHAR(2) NULL COMMENT '등급(A+,A,A-,...,F), NULL 허용',
  `midterm_score` DECIMAL(5,2) NULL COMMENT '중간고사 점수(0~100)',
  `final_score`   DECIMAL(5,2) NULL COMMENT '기말고사 점수(0~100)',

  CONSTRAINT `PK_GRADES` PRIMARY KEY (`enrollment_id`),
  ...
  -- 점수 범위 체크 (NULL 허용)
  CONSTRAINT `CK_GRADE_MIDTERM_SCORE`
    CHECK ( `midterm_score` IS NULL OR (`midterm_score` >= 0 AND `midterm_score` <= 100) ),
  CONSTRAINT `CK_GRADE_FINAL_SCORE`
    CHECK ( `final_score`  IS NULL OR (`final_score`  >= 0 AND `final_score`  <= 100) )
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='수강별 성적(중간/기말/등급)';
...
CREATE TABLE `chatting` (
  ...
  `created_date` DATE          NOT NULL,                           -- 작성 '날짜'
  `create_hours` TIME          NOT NULL,                           -- 작성 '시간(HH:MM:SS)'
  `content`      TEXT          NOT NULL,                           -- 내용
  `file_path`    VARCHAR(300)  NULL,                               -- 첨부 경로
  `author_id`    INT UNSIGNED  NOT NULL,                           -- 작성자 FK
...
CREATE TABLE `Leave_of_Absence` (
  `leave_id`        INT UNSIGNED      NOT NULL AUTO_INCREMENT COMMENT '휴학 신청 PK',
  `student_id`      INT UNSIGNED      NOT NULL COMMENT '학생 FK',
  `reason`          TEXT              NOT NULL COMMENT '휴학 사유',
  `applied_at`      DATETIME          NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '휴학 신청 일시',
  `leave_year`      SMALLINT UNSIGNED NOT NULL COMMENT '휴학 년도(YYYY)',
  `leave_semester`  TINYINT  UNSIGNED NOT NULL COMMENT '휴학 학기(1:1학기, 2:2학기)',
  `leave_type`      VARCHAR(10)       NOT NULL COMMENT '휴학 종류(일반휴학/군휴학/질병휴학/기타)',
  /* ← 여기 있던 leave_term 가상 컬럼 제거 */

  /* On-leave flag: T (on leave) / F (not on leave) */
  `is_on_leave`     CHAR(1)           NOT NULL DEFAULT 'T' COMMENT 'T: on leave, F: not on leave',

  CONSTRAINT `PK_LOA` PRIMARY KEY (`leave_id`),
  CONSTRAINT `FK_LOA_STUDENT`
    FOREIGN KEY (`student_id`) REFERENCES `student`(`student_id`)
      ON DELETE RESTRICT ON UPDATE CASCADE,
  CONSTRAINT `CK_LOA_TYPE`
    CHECK (`leave_type` IN ('일반휴학','군휴학','질병휴학','기타')),
  CONSTRAINT `CK_LOA_SEM`
    CHECK (`leave_semester` IN (1,2)),
  CONSTRAINT `CK_LOA_YEAR_FMT`
    CHECK (`leave_year` BETWEEN 2000 AND 2100),
  CONSTRAINT `CK_LOA_IS_ON_LEAVE`
    CHECK (`is_on_leave` IN ('T','F'))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;


CREATE TABLE `back_school` (
  `return_id`     INT UNSIGNED  NOT NULL AUTO_INCREMENT,  -- 복학 PK
  `leave_id`      INT UNSIGNED  NOT NULL,                 -- FK → Leave_of_Absence
  `manager`         INT UNSIGNED  NULL,                     -- FK → employee (NULL 허용)
  `return_reason` TEXT          NOT NULL,                 -- 복학 사유
  `return_date`   DATE          NOT NULL,                 -- 복학 날짜
  `is_returned`   VARCHAR(1)    NOT NULL,                 -- 복학 여부 't'/'f'

  CONSTRAINT `PK_RFL` PRIMARY KEY (`return_id`),
  CONSTRAINT `FK_RFL_LOA`
    FOREIGN KEY (`leave_id`) REFERENCES `Leave_of_Absence`(`leave_id`)
      ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK_RFL_EMP`
    FOREIGN KEY (`manager`) REFERENCES `employee`(`employee_id`)
      ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT `CK_RFL_IS_RETURNED`
    CHECK (`is_returned` IN ('t','f'))
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
