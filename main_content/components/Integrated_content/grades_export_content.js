  ...
  const handleScoreChange = (id, scoreType, value) => {
    ...
    setStudents(currentStudents =>
      currentStudents.map(student =>
        student.id === id ? { ...student, scores: { ...student.scores, [scoreType]: num } } : student
      )
    );
  };
        ...
        <tbody>
          {processedStudents.map(student => (
            <tr key={student.id}>
              ...
              <td>{student.name}</td>
              {['mid', 'final', 'assign', 'attend'].map(type => (
                <td key={type}>
                  <input
                    type="number"
                    min="0"
                    max="100"
                    value={student.scores[type]}
                    className={
                      student.scores[type] < 0 || student.scores[type] > 100
                        ? styles.invalid
                        : styles.valid
                    }
