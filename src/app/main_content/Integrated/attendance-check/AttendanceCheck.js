...
/**
 * 월별 출결 데이터를 API에서 가져옵니다.
 * @param {string} dateStr - 현재 날짜 (YYYY-MM-DD)
 * @param {string} currentCourseId - 현재 선택된 강의 ID
 * @returns {Promise<Object>} 캘린더 변환 데이터
 */
...
// 2. API 함수 (출결 데이터 조회)
// ----------------------------------------------------
const fetchAttendanceData = async (dateStr, currentCourseId) => {
    // 1. Next.js API Routes의 실제 경로
    const API_PATH = '/api/univer_city/attendance_check'; 
    // 현재는 코스 ID를 임의로 1001로 가정. 실제로는 state/prop으로 관리되어야 함.

    if (!currentCourseId) {
        return []; // 강의 선택이 안 된 경우
    }

    // 2. 쿼리 파라미터 생성: courseId와 YYYY-MM 형식의 month를 전달
    const apiUrl = `${API_PATH}?courseId=${currentCourseId}&month=${dateStr.substring(0, 7)}`;
    ...
    try {
        ...
        // 3. 데이터를 캘린더 형식에 맞게 변환
        // (transformAttendanceData 함수는 기존에 있으므로 그대로 사용)
        return transformAttendanceData(rawData);
    } catch (error) {
        console.error('월별 출결 데이터 가져오기 실패:', error);
        return {}; // 실패 시 빈 객체 반환
    }
};

// ⭐️ [신규 함수] 날짜별 학생 리스트 데이터를 API로 가져오는 모의 함수 (모달용)
const fetchDailyStudentAttendance = async (dateStr) => {
    // 1. route2.js의 API 경로 사용
    const API_PATH = '/api/univer_city/attendance_data'; 
    // 현재는 코스 ID를 임의로 1001로 가정. 실제로는 state/prop으로 관리되어야 함.
    const currentCourseId = 1001; 

    if (!currentCourseId) {
        return [];
    }
    
    // 2. 쿼리 파라미터 생성: lectureId(courseId)와 YYYY-MM-DD 형식의 date를 전달
    const apiUrl = `${API_PATH}?lectureId=${currentCourseId}&date=${dateStr}`;
    ...
    try {
        ...
        // 3. 응답 데이터를 모달이 요구하는 형식으로 반환
        return studentData;
    } catch (error) {
        console.error(`일별 상세 출결 데이터(${dateStr}) 가져오기 실패:`, error);
        return []; // 실패 시 빈 배열 반환
    }
};
        ...
        <div className={styles.layout}>
            ...
            <div className={styles.topbar}>
                ...
                <div className={styles.titleRow}>
                    ...
                    >
                        <option value="컴소과 2-1 자료구조">컴소과 2-1 자료구조</option>
