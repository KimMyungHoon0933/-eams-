...
export function BackSchoolApply() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      // 2) INSERT 파라미터 구성 (라우터 규약에 맞게)
      //    테이블/컬럼명은 네 DB에 맞춰 변경 가능
      const TABLE_NAME = "return_apply";
      const cols = [
        "student_id","name","dept","phone","email","addr",
        "leave_type","return_term","reason","credits","memo"
      ];
      const values = [
        data.studentId, data.name, data.dept, data.phone, data.email, data.addr,
        data.leaveType, data.returnTerm, data.reason, data.credits, data.memo
      ];

      // 3) GET /api/insert?table=...&cols=[...]&values=[...]
      const qs = new URLSearchParams({
        table: TABLE_NAME,
        cols: JSON.stringify(cols),
        values: JSON.stringify(values) // 단건 → 1차원 배열 허용
      });
      ...
      if (!res.ok) {
        console.error(json);
        alert(`DB 저장 실패: ${json.error || "Unknown error"}`);
        return;
      }

      // 4) 성공 처리
      setStatus("제출됨");
      alert(`제출되었습니다.\n[SQL] ${json.sql}\ninsertId: ${json.insertId ?? "N/A"}`);
      // 진행상태 페이지로 이동하고 싶다면 여기서 router.push(...) 사용
    } catch (err) {
      console.error(err);
