...
export async function deleteSession(sid) {
  ...
  // session_id + token_hash 가 동시에 일치하는 세션만 삭제
  const where = encodeURIComponent(
    `session_id = '${session_id}' AND token_hash = '${token_hash}'`
  );

  // 🔥 여기서 사용하는 delete 라우트 이름은
  // 프로젝트에서 실제로 만든 경로 이름에 맞게 필요하면 수정해줘
  const url = `/api/univer_city/delete_where_route?table=session&where=${where}`;
